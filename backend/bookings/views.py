...
from .serializers import (
    CreateBookingSerializer, BookingCalculationSerializer,
    BookingListSerializer, BookingDetailSerializer
)
from .services import RentCalculationService, AvailabilityService, ReceiptGeneratorService
    ...
    def post(self, request):
        ...
        try:
            ...
            booking = Booking.objects.create(
                ...
                id_proof_type=applicant['id_proof_type'].lower().replace(' ', '_').replace('_card', ''),
            ...
            return Response({'success': False, 'message': 'Booking failed. Please try again.'}, status=500)
    ...
    def get(self, request):
        ...
        try:
            booking = Booking.objects.get(
                Q(booking_id=query) | Q(mobile=query)
            )
            return Response({'success': True, 'data': BookingDetailSerializer(booking).data})
        except Booking.DoesNotExist:
            return Response({'success': False, 'message': 'No booking found.'}, status=404)
        except Booking.MultipleObjectsReturned:
            bookings = Booking.objects.filter(mobile=query).order_by('-created_at')
            return Response({'success': True, 'data': BookingListSerializer(bookings, many=True).data})
...
class ReceiptDownloadView(APIView):
    ...
    def get(self, request, booking_id):
        try:
            ...
            return Response({'message': 'Booking not found or not approved.'}, status=404)
...
class AdminBookingListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return Booking.objects.all().select_related('premise', 'slot').order_by('-created_at')
    ...
    def post(self, request, booking_id):
        try:
            ...
            if booking.status != 'pending':
                return Response({'message': f'Cannot approve. Status: {booking.status}'}, status=400)
...
class AdminDuplicateReceiptView(APIView):
    ...
    def get(self, request, booking_id):
        try:
            ...
            pdf_buffer = ReceiptGeneratorService.generate_pdf(booking)
            response = HttpResponse(pdf_buffer, content_type='application/pdf')
            response['Content-Disposition'] = f'attachment; filename="DuplicateReceipt_{booking_id}.pdf"'
            return response
        except Booking.DoesNotExist:
            return Response({'message': 'Booking not found.'}, status=404)
