...
from premises.models import Holiday, Premise, PremiseRate, TimeSlot
...
class AvailabilityService:
    ...
    def is_slot_available(
        premise_id: int, slot_id: int, start_date: date, end_date: date
    ) -> bool:
...
class RentCalculationService:
    ...
    @classmethod
    def calculate(
        cls, premise_id: int, slot_id: int, start_date: date, end_date: date
    ) -> dict:
...
class ReceiptGeneratorService:
    ...
    def generate_pdf(booking: Booking) -> io.BytesIO:
        ...
        story.append(
            Paragraph("Hutatma Smruti Mandir — Booking Receipt", styles["Heading2"])
        )
        ...
        data = [
            ...
            [
                "Total Days",
                str(booking.total_days),
                "Slot",
                booking.slot.name if booking.slot else "N/A",
            ],
        ]
        ...
        tbl2 = Table(
            applicant_data, colWidths=[1.5 * inch, 2.5 * inch, 1.2 * inch, 2 * inch]
        )
        tbl2.setStyle(
            TableStyle(
                [
        ...
        story.append(
            Paragraph(
                "<i>This is a computer-generated receipt. | Solapur Municipal Corporation</i>",
                styles["Normal"],
            )
        )
