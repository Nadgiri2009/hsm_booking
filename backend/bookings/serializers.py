...
class ApplicantDetailsSerializer(serializers.Serializer):
    ...
    mobile = serializers.RegexField(
        regex=r"^[6-9]\d{9}$", error_messages={"invalid": "Invalid mobile number"}
    )
    alt_mobile = serializers.RegexField(
        regex=r"^[6-9]\d{9}$", required=False, allow_blank=True
    )
...
class BankDetailsSerializer(serializers.Serializer):
    ...
    ifsc = serializers.RegexField(
        regex=r"^[A-Z]{4}0[A-Z0-9]{6}$", error_messages={"invalid": "Invalid IFSC code"}
    )
    ...
    def validate(self, data):
        ...
        if not AvailabilityService.is_slot_available(
            ...
            raise serializers.ValidationError(
                "The selected slot is not available for these dates."
            )
