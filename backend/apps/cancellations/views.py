...
def calculate_refund_percentage(event_date):
    ...
    if days_left >= 60: return 90
    elif days_left >= 30: return 80
    elif days_left >= 7: return 50
    else: return 0  # security deposit only
...
class CancellationRequestView(APIView):
    ...
    def post(self, request):
        ...
        cancellation = Cancellation.objects.create(
            ...
            refund_percentage=refund_pct, refund_amount=refund_amount,
            otp=otp, otp_expiry=timezone.now() + timedelta(minutes=10)
        )
        # TODO: Send OTP via SMS/Email
        return Response({'message': 'OTP sent to registered mobile/email', 'cancellation_id': cancellation.id, 'refund_percentage': refund_pct, 'refund_amount': refund_amount})
    ...
    def post(self, request):
        ...
        try:
            ...
            if cancellation.otp == otp and cancellation.otp_expiry > timezone.now():
                ...
                cancellation.save()
                return Response({'message': 'Cancellation verified successfully. Refund will be processed after admin approval.'})
