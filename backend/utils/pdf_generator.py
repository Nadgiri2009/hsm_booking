...
def _amount_in_words(value):
    rounded_value = int(
        _as_decimal(value).quantize(Decimal("1"), rounding=ROUND_HALF_UP)
    )
...
def generate_receipt_pdf(booking):
    ...
    title_style = ParagraphStyle(
        "title", parent=centered, fontName="Helvetica-Bold", fontSize=11
    )
    subtitle_style = ParagraphStyle(
        "subtitle", parent=centered, fontName="Helvetica-Bold", fontSize=10
    )
    ...
    transaction_id = (
        payment.transaction_ref if payment and payment.transaction_ref else "-"
    )
    payment_mode = (
        payment.payment_mode
        if payment and payment.payment_mode
        else booking.payment_mode
    )
    ...
    receipt_content.append(
        _section_table("Applicant Details", applicant_rows, doc.width)
    )
    ...
    receipt_content.append(
        HRFlowable(width="100%", thickness=1, color=colors.HexColor("#555555"))
    )
    ...
    receipt_content.append(
        Paragraph(
            ...
            ParagraphStyle(
                "footer_title", parent=centered, fontName="Helvetica-Bold", fontSize=9
            ),
        )
    )
    receipt_content.append(
    ...
    receipt_content.append(
        Paragraph(f"hsm@solapurcorporation.gov.in, {OFFICE_ADDRESS}", normal_center)
    )
    generated_on = timezone.localtime()
    receipt_content.append(
        Paragraph(
            f"Generated On: {generated_on.strftime('%m/%d/%Y, %I:%M:%S %p')}",
            normal_center,
        )
    )
